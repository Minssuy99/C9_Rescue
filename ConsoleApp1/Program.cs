...
public class GameManager
{
    ...
    private void StatusMenu()
    {
        ...
        // 장착되기 전의 기본 능력치
        int baseAtk = player.BaseAtk;
        int baseDef = player.BaseDef;
        int baseHp = player.BaseHp;

        // 장착된 아이템의 능력치 강화된 부분 추가하기
        int bonusAtk = inventory.Select(item => item.IsEquipped ? item.Atk : 0).Sum();
        int bonusDef = inventory.Select(item => item.IsEquipped ? item.Def : 0).Sum();
        int bonusHp = inventory.Select(item => item.IsEquipped ? item.Hp : 0).Sum();

        // 기본 능력치에 더해 출력
        ConsoleUtility.PrintTextHighlights("공격력 :", (baseAtk + bonusAtk).ToString(), bonusAtk > 0 ? $"(+{bonusAtk})" : "");
        ConsoleUtility.PrintTextHighlights("방어력 :", (baseDef + bonusDef).ToString(), bonusDef > 0 ? $"(+{bonusDef})" : "");
        ConsoleUtility.PrintTextHighlights("체  력 :", (baseHp + bonusHp).ToString(), bonusHp > 0 ? $"(+{bonusHp})" : "");
    ...
    private void EquipMenu()
    {
        ...
        switch (keyInput)
        {
            case 0:
                ...
                break;
            default:
                inventory[keyInput - 1].ToggleEquipStates(inventory, player);
                EquipMenu();
                break;
        }
    }
